...
class ASPTranslator:
    ...
    def _fmt_method_head(self, method, time_var):
        """Format method head with compound task name, method parameters, and time."""
        name = method['name'].replace('-', '_')

        # Nur der Name der Compound Task (ohne Parameter)
        task_name = ""
        if method['task']:
            task_name = method['task'][0].replace('-', '_')

        # Parameter der Methode formatieren
        params = [self._fmt_term(p[0]) for p in method['params']]
        params_str = ", ".join(params)

        if task_name and params_str:
            return f"{name}({task_name}, {params_str}, {time_var})"
        elif task_name:
            return f"{name}({task_name}, {time_var})"
        elif params_str:
            return f"{name}({params_str}, {time_var})"
        else:
    ...
    def _get_method_typing_constraints(self, method):
        ...
        for param_name, param_type in method['params']:
            if param_type != 'object':
                constraints.append(f"{param_type}({self._fmt_term(param_name)})")
        return constraints
            ...
            for method in methods:
                        ...
                        for i, param in enumerate(task_params_in_call):
                            if param.startswith('?') and param not in method_param_names:
                                ...
                                if i < len(task_param_defs):
                                    ...
                                    if typ != 'object':
                                        body_parts.append(f"{typ}({self._fmt_term(param)})")
                ...
                for other_method in methods:
                    if other_method['name'] != method['name']:
                        other_head = self._fmt_method_head(other_method, "T")
    ...
    def translate_problem(self):
        ...
        for i, task in enumerate(self.data.htn_tasks):
            ...
            task_name = task[0]
            is_primitive = task_name in self.data.actions
            ...
            if i == 0:
                ...
                if i == 1:
                    causable_call = f"causable({prev_task_atom}, 0, T1)"
                    rule = f"taskTBA({task_atom}, T1) :- {causable_call}, T1 >= 0, time(T1)."
                else:
        ...
        if self.data.htn_tasks:
            ...
            last_task_name = last_task[0]
            last_is_primitive = last_task_name in self.data.actions
